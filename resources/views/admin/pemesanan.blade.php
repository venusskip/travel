...
<body class="bg-[#F8F9FA] flex min-h-screen font-sans">
    ...
    <main class="flex-1 p-8">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
            ...
            @if(session('success'))
    <div class="mb-4 p-4 bg-emerald-50 border border-emerald-200 text-emerald-700 rounded-xl text-xs font-medium">
        {{ session('success') }}
    </div>
@endif
    ...
    <div class="flex justify-center items-center space-x-1">
        ...
        <form action="{{ route('admin.booking.destroy', $booking->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pesanan ini? Aksi ini akan mengosongkan kembali nomor kursi terkait.')" class="inline">
            @csrf
            @method('DELETE')
            <button type="submit" class="text-red-500 hover:text-red-700 p-2 rounded-lg hover:bg-red-50 transition" title="Hapus Pemesanan">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
