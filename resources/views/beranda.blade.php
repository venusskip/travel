...
<body class="bg-gray-50 text-gray-900 antialiased">
    ...
    <section class="bg-gradient-to-b from-[#1e50da] via-[#1b49c4] to-[#153794] text-white pt-24 pb-56 relative">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            ...
            <form action="/jadwal" method="GET" class="absolute -bottom-28 left-4 right-4 md:left-8 md:right-8 max-w-4xl bg-white p-4 rounded-[20px] shadow-[0_15px_30px_rgba(0,0,0,0.08)] text-gray-700 flex flex-col md:flex-row gap-4 items-center">
    ...
    <div class="w-full md:flex-1 text-left">
        ...
        <div class="relative">
            <select name="asal" class="w-full bg-gray-50 border border-gray-100 rounded-xl pl-4 pr-10 py-3 text-sm focus:outline-none focus:border-blue-500 focus:bg-white font-medium text-gray-600 appearance-none transition-all">
    ...
    <div class="w-full md:flex-1 text-left">
        ...
        <div class="relative">
            <select name="tujuan" class="w-full bg-gray-50 border border-gray-100 rounded-xl pl-4 pr-10 py-3 text-sm focus:outline-none focus:border-blue-500 focus:bg-white font-medium text-gray-600 appearance-none transition-all">
            ...
            <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-4 text-gray-400">
                <svg class="fill-current h-4 w-4 opacity-70" xmlns="http://www.w3.org/2000/xl" viewBox="0 0 20 20">
                    <path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/>
                </svg>
            </div>
        </div>
    ...
    <div class="w-full md:w-auto pt-5 md:pt-0">
        <button type="submit" class="w-full md:w-auto bg-[#2260ff] hover:bg-blue-700 text-white font-medium py-3.5 px-8 rounded-xl text-sm flex items-center justify-center space-x-2 transition shadow-md shadow-blue-600/10 whitespace-nowrap">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-5xl mx-auto">
        ...
        <a href="/jadwal?layanan=Travel Antar Kota" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-blue-200 transition block">
        ...
        <a href="/jadwal?layanan=Travel Wisata" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-blue-200 transition block">
        ...
        <a href="/jadwal?layanan=Travel Eksekutif" class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 hover:shadow-md hover:border-blue-200 transition block">
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-20">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            @if($schedules->isEmpty())
                <div class="col-span-1 md:col-span-3 text-center py-12 text-gray-400 text-sm bg-white rounded-2xl border border-gray-100 shadow-sm">
                    Belum ada jadwal travel yang tersedia saat ini.
                </div>
            @else
                @foreach($schedules as $schedule)
                <a href="/detail-travel/{{ $schedule->id }}" class="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-md transition group block">
                    <div class="bg-blue-50 h-28 flex items-center justify-center text-blue-300 group-hover:bg-blue-100 transition relative overflow-hidden">
                        @if($schedule->foto_kendaraan)
                            <img src="{{ asset('storage/' . $schedule->foto_kendaraan) }}" class="w-full h-full object-cover" alt="Foto {{ $schedule->nama_travel }}">
                        @else
                    ...
                    <div class="p-5">
                        <h3 class="font-bold text-sm text-slate-800 mb-2">{{ $schedule->nama_travel }}</h3>
                        <div class="text-xs text-gray-400 space-y-1.5 mb-5">
                            <div class="flex items-center space-x-2">
                                ...
                                <span class="font-medium text-gray-600">{{ $schedule->kota_asal }} — {{ $schedule->kota_tujuan }}</span>
                            </div>
                            <div class="flex items-center space-x-2">
                                ...
                                <span>{{ $schedule->tanggal_berangkat ? $schedule->tanggal_berangkat->format('d M Y') : '-' }}  •  {{ $schedule->jam_berangkat }}</span>
                            </div>
                            <div class="flex items-center pt-0.5">
                                <span class="bg-blue-50 text-blue-600 text-[10px] px-2 py-0.5 rounded font-semibold uppercase tracking-wide">{{ $schedule->jenis_layanan }}</span>
                            </div>
                        </div>
                        <div class="flex justify-between items-center pt-3 border-t border-gray-50">
                            <span class="text-blue-600 font-bold text-sm">Rp {{ number_format($schedule->harga, 0, ',', '.') }}</span>
                            
                            @php
                                $sisaKursi = $schedule->total_kursi - $schedule->kursi_terisi;
                            @endphp
                            
                            @if($sisaKursi > 0)
                                <span class="bg-emerald-50 text-emerald-600 text-[10px] px-2 py-1 rounded font-medium">{{ $sisaKursi }} kursi tersisa</span>
                            @else
                                <span class="bg-red-50 text-red-600 text-[10px] px-2 py-1 rounded font-medium">Habis</span>
                            @endif
                        </div>
                    </div>
                </a>
